...
// characters.
//
int echo_enable = 1; // 1 la bat hien thi, 0 la tat
void
consputc(int c)
{
  if(c == BACKSPACE){
...
consoleintr(int c)
{
  ...
  default:
	...
	if(echo_enable == 1) {      // <--- THÊM DÒNG NÀY (Mở ngoặc kiểm tra công tắc)
              consputc(c);
          }                           // <--- THÊM DÒNG NÀY (Đóng ngoặc)	
...
uint64 sys_setecho(void) {
    int state;
    
    // Gọi thẳng hàm argint, không cần so sánh if nữa
    argint(0, &state); 
